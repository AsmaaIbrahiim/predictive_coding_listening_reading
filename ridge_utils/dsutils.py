...
def make_semantic_model(datasequence, lsasm):
    newdata = []
    for word in datasequence.data:
        try:
            v = lsasm[str.encode(word.lower())]
        except KeyError as e:
            v = np.zeros((lsasm.data.shape[0],))
        newdata.append(v)
    return DataSequence(np.array(newdata), datasequence.split_inds, datasequence.data_times, datasequence.tr_times)
