...
def ridge(stim, resp, alpha, singcutoff=1e-10, normalpha=False, logger=ridge_logger):
    ...
    try:
        ...
        from ridge_utils.svd_dgesvd import svd_dgesvd
...
def bootstrap_ridge(Rstim, Rresp, Pstim, Presp, alphas, nboots, chunklen, nchunks,
    ...
    if return_wt:
        ...
        ridge_dict = {"Rstim": Rstim,  "Rresp": Rresp, "valphas": valphas, "normalpha": normalpha, "singcutoff": singcutoff}
        with open('ridge.pickle', 'wb') as pickle_file:
            pickle.dump(ridge_dict, pickle_file)


        logger.info("saved the important files")
